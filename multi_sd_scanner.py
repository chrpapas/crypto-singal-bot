    ...
    for tr in ps["open_trades"]:
        ...
        try:
            ...
            if assume == "signal_close":
                eval_rows = rows[1:]  # after entry bar anyway
            else:  # next_close
                # treat first row as the bar where we take the next close fill
                if len(rows) >= 2:
...
# ======== Merge helpers for logs ========
def _roundf(x, n=6):
    try: return float(round(float(x), n))
    except Exception: return None
def _entry_key(s: dict):
    return (s.get("type"), s.get("symbol"), s.get("timeframe"), s.get("note",""),
            _roundf(s.get("entry")),_roundf(s.get("stop")),_roundf(s.get("t1")),_roundf(s.get("t2")),_roundf(s.get("level")))
def _exit_key(s: dict): return (s.get("type"), s.get("symbol"), s.get("timeframe"), s.get("reason",""))
def _sell_key(s: dict): return (s.get("type"), s.get("symbol"), s.get("timeframe"), s.get("note",""), _roundf(s.get("price")), _roundf(s.get("level")))
...
def run(cfg: Dict[str,Any]):
                ...
                try:
                    ...
                    if ok:
                        ...
                        if str(bar_ts) != last_bar:
                            ...
                            # performance: neutral-close open day trades on this pair/TF
    ...
    # ===== Render logs =====
    entries_view = _merge_by_exchange(results.get("signals", []), _entry_key)
    ...
    if entries_view:
        for s in entries_view:
            sd_tag = " ✅SD" if s.get("sd_confluence") else ""
            conf = f" (conf {s.get('confidence',0)})" if "confidence" in s else ""
            exs = ",".join(sorted(set(s.get("exchanges", []))))
            print(f"[ENTRY] [{exs}] {s['symbol']} {s['timeframe']} {s['type'].upper()} — {s['note']}{sd_tag}{conf} — entry {s['entry']} stop {s['stop']} t1 {s['t1']} t2 {s['t2']}")
    else:
            ...
            try:
                ...
                if entries_view:
                    ...
                    for s in entries_view:
                        ...
                        sd_tag=" ✅SD" if s.get("sd_confluence") else ""
                        conf = f" (conf {s.get('confidence',0)})" if "confidence" in s else ""
                        parts.append(f"• [{exs}] `{s['symbol']}` *{s['type'].upper()}* {s['timeframe']} — {s['note']}{sd_tag}{conf}\n  entry `{s['entry']}` stop `{s['stop']}` t1 `{s['t1']}` t2 `{s['t2']}`")
                if exits_view:
                    parts.append("*Exits*")
                    for x in exits_view:
                        exs = ", ".join(sorted(set(x.get("exchanges", []))))
            ...
            try:
                ...
                if entries_view:
                    ...
                    for s in entries_view:
                        ...
                        sd_tag=" ✅SD" if s.get("sd_confluence") else ""
                        conf = f" (conf {s.get('confidence',0)})" if "confidence" in s else ""
                        lines.append(f"**[{exs}] {s['symbol']}** ({s['timeframe']} {s['type'].upper()}) — {s['note']}{sd_tag}{conf}\nentry `{s['entry']}` stop `{s['stop']}` t1 `{s['t1']}` t2 `{s['t2']}`")
                if exits_view:
                    lines.append("**Exits**")
                    for x in exits_view:
                        exs = ", ".join(sorted(set(x.get("exchanges", []))))
