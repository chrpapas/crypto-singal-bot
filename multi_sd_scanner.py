...
- Scan one or more exchanges (comma-separated EXCHANGES env)
- Day trade signals (1h) for your symbols + CMC movers (if enabled)
- Trend trade signals (1d) for your symbols
...
Start command for Render Cron:
  python multi_sd_scanner.py --config config.yml
"""
...
class ExClient:
    ...
    def has_pair(self, symbol_pair: str) -> bool:
        """Safe membership test even if ccxt sets .symbols to None."""
        mkts = self.load_markets() or {}
        if symbol_pair in mkts:
            return True
        symbols = getattr(self.ex, "symbols", None) or []
        return symbol_pair in symbols
...
def find_zones(df: pd.DataFrame, impulse_factor=1.8, zone_padding_pct=0.25, max_age_bars=300):
    """Simple SD zone detector using 'opposite candle before impulse' heuristic."""
    zones = []
    ar = avg_range(df, 20)
    b = body(df)
...
def day_signal(df: pd.DataFrame, p: DayParams, sd_cfg: Dict[str,Any], zones=None):
    ...
    if not (breakout or retrec):
        return None

    # SD gating
    if sd_cfg.get("enabled") and sd_cfg.get("mode","prefer") == "require":
        if not in_demand(float(last["close"]), zones):
            return None
    ...
    sig = {
        ...
        "level": float(highlvl),
        "note": "Breakout" if breakout else "Retest-Reclaim",
    }
    if sd_cfg.get("enabled"):
        sig["sd_confluence"] = in_demand(entry, zones)
    return sig
...
def trend_signal(df: pd.DataFrame, p: TrendParams, sd_cfg: Dict[str,Any], zones=None):
    ...
    within  = abs((last["close"] - last["ema20"]) / last["ema20"] * 100.0) <= p.pullback_pct_max
...
def btc_ok(ex: ExClient, dayp: DayParams) -> bool:
    try:
        ...
        # If BTC fetch fails, don't block signals
        return True


# ---------------- Movers via CoinMarketCap ----------------
def fetch_cmc_list(cfg: Dict[str,Any]) -> List[str]:
    ...
    min_vol    = mv.get("min_volume_usd_24h", 2_000_000)
    ...
    for it in data:
        ...
        if sym in exclude:
            continue
        if include and (sym in include):  # optional, but you can leave include blank
            out.append(sym)
            continue
...
def filter_pairs_on_exchanges(ex_clients: Dict[str,ExClient], symbols: List[str], quote: str = "USDT") -> Dict[str,List[str]]:
    """Return dict {exchange_name: [ 'SYM/USDT', ... ]} with only tradable pairs on each exchange."""
...
def run(cfg: Dict[str,Any]):
    ...
    # movers list → tradable per exchange
    ...
    if tcfg.get("enabled") and results["signals"]:
        try:
            ...
            for s in results["signals"]:
                sd_tag = " ✅SD" if s.get("sd_confluence") else ""
                lines.append(
                    f"• [{s['exchange']}] `{s['symbol']}` *{s['type'].upper()}* {s['timeframe']} — {s['note']}{sd_tag}\n"
                    f"  entry `{s['entry']}` stop `{s['stop']}` t1 `{s['t1']}` t2 `{s['t2']}`"
                )
            requests.post(
                f"https://api.telegram.org/bot{tcfg['bot_token']}/sendMessage",
...
if __name__ == "__main__":
    ...
    # Expand env placeholders for top-level keys
