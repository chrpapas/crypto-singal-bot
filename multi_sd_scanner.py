...
print("=== multi_sd_scanner v2025-10-20 r3 (patched) ===", flush=True)
...
# --------- Exchange wrapper (strict has_pair + safe OHLCV) ---------
class ExClient:
    ...
    def load_markets(self):
        if self._markets is None:
            try: self._markets = self.ex.load_markets()
                ...
                print(f"[{self.name}] load_markets err: {e}", flush=True)
                self._markets = {}
    ...
    def has_pair(self, symbol_pair: str) -> bool:
        try:
            ...
            # strict lookup
            self.ex.market(symbol_pair)  # raises if not found
            return True
        except Exception:
            return False
...
def run(cfg: Dict[str,Any]):
    ...
    if any(x.startswith("${") and x.endswith("}") for x in ex_names):
        print("[error] EXCHANGES not set. Set env EXCHANGES or put explicit list in config.yml.", flush=True); return
    if any(x.startswith("${") and x.endswith("}") for x in watchlist):
        print("[error] SYMBOLS_WATCHLIST not set. Set env SYMBOLS_WATCHLIST or put explicit list in config.yml.", flush=True); return
    ...
    for ex_name, client in ex_clients.items():
        # strict per-exchange filtering
        base_pairs: List[str] = []
        for p in watchlist:
            if client.has_pair(p): base_pairs.append(p)
            else: print(f"[skip] {ex_name} does not list {p}", flush=True)

        # iterate filtered pairs
        for pair in base_pairs:
            try:
                df1h = client.ohlcv(pair, "1h", 200)
                ok, why, bar_ts = day_exit_edge(df1h, exits_cfg.get("day", {}))
                if ok: print(f"[EXIT ] {ex_name} {pair} 1h — {why}", flush=True)
                else: print(f"[OK   ] {ex_name} {pair} 1h — no exit", flush=True)
            except ccxt.BaseError as e:
                print(f"[exits] {ex_name} {pair} ccxt err: {e}", flush=True)
            except Exception as e:
