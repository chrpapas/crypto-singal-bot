...
- Top-100-by-marketcap scan (signals; optional ETH gate for bullish)
- Legacy Movers scan (signals; ETH gate does NOT apply)
- Bear-safe flow:
    • Bearish detectors (1h/4h/1d) always alert, even if no inverse token
    • If inverse 3S/5S exists -> long that symbol for trading
    • Canonical silent key on UNDERLYING to prevent duplicates
    • Auto close (invalidate) bear alerts when conditions flip
    • Trigger "bear exits": close any open long positions on the same underlying
- Multi-timeframe confirm for day signals (5m/15m/30m/45m)
- Stablecoin/pegged filtering (base-only)
- Silent-signal registry (no duplicates until t1/stop/invalidate)
- Paper/live execution with risk sizing and max-open guard
- Discord: Signals (Top100 + Movers + Bear) + Trades (only when executed)
- Paper snapshot
...
def avg_range(df: pd.DataFrame, n: int = 20) -> pd.Series:
...
def day_signal(df: pd.DataFrame, p: DayParams, sd_cfg: Dict[str, Any], zones=None):
    ...
    breakout_ok = (
        ...
        and (p.rsi_min <= r.iloc[-1] <= p.rsi_max)
    )
    ...
    retrec_ok = (
        ...
        and (r.iloc[-1] >= p.rsi_min)
    )
    if not (breakout_ok or retrec_ok):
        return None
    if sd_cfg.get("enabled") and sd_cfg.get("mode", "prefer") == "require" and not in_demand(float(last["close"]), zones):
        return None
    entry = float(last["close"])
    stop = stop_from(df, p.stop_mode, p.atr_mult)
    return {
        "type": "day",
        "entry": entry,
        "stop": stop,
...
def swing_signal(df: pd.DataFrame, p: Dict[str, Any], sd_cfg: Dict[str, Any], zones=None):
    ...
    if len(df) < need:
        return None
    df = df.copy()
    df['ema20'] = ema(df['close'], p.get('ema20', 20))
    df['ema50'] = ema(df['close'], p.get('ema50', 50))
    df['ema100'] = ema(df['close'], p.get('ema100', 100))
    df['volS'] = sma(df['volume'], p.get('vol_sma', 20))
    r = rsi(df['close'], 14)
    last = df.iloc[-1]
    ...
    if sd_cfg.get('enabled') and sd_cfg.get('mode', 'prefer') == 'require' and not in_demand(float(last['close']), zones):
        return None
    entry = float(last['close'])
    stop = stop_from(df, p.get('stop_mode', 'swing'), p.get('atr_mult', 2.0))
    return {
        "type": "swing",
        "entry": entry,
        "stop": stop,
        "t1": round(entry * 1.06, 6),
        "t2": round(entry * 1.12, 6),
        "level": float(hl),
...
def trend_signal(df: pd.DataFrame, p: TrendParams, sd_cfg: Dict[str, Any], zones=None):
    ...
    if len(df) < need:
        return None
    df = df.copy()
    df["ema20"] = ema(df["close"], p.ema20)
    df["ema50"] = ema(df["close"], p.ema50)
    df["ema100"] = ema(df["close"], p.ema100)
    df["volS"] = sma(df["volume"], p.vol_sma)
    r = rsi(df["close"], 14)
    last = df.iloc[-1]
    ...
    return {
        ...
        "entry": entry,
        "stop": stop,
        "t1": round(entry * 1.08, 6),
        "t2": round(entry * 1.20, 6),
        "level": float(hl),
...
def tf_bull_ok(df: pd.DataFrame, *, require_ema_stack=True, require_macd_bull=True, min_rsi=50) -> bool:
    ...
    if require_ema_stack:
        ok &= (e20.iloc[-1] > e50.iloc[-1])
    if require_macd_bull:
        ok &= (macd_line.iloc[-1] > macd_sig.iloc[-1])
    ok &= (r.iloc[-1] >= min_rsi)
    return bool(ok)
...
def day_bearish_signal(df: pd.DataFrame, cfg: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    tf_cfg = cfg.get("day", {})
    look = int(tf_cfg.get("lookback_low", 20))
    voln = int(tf_cfg.get("vol_sma", 20))
    if len(df) < max(look, voln) + 5:
        return None
    lowlvl = df["low"].iloc[-(look + 1):-1].min()
    last = df.iloc[-1]
    volS = sma(df["volume"], voln)
    r = rsi(df["close"], 14)
    ...
    if not (breakdown and vol_ok and rsi_weak):
        return None
    entry = float(last["close"])
...
def swing_bearish_signal(df: pd.DataFrame, cfg: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    breakdown = last["close"] < lowlvl
    rsi_weak = r.iloc[-1] <= tf_cfg.get("rsi_max", 50)
    vol_ok = last["volume"] > (volS.iloc[-1] if not np.isnan(volS.iloc[-1]) else last["volume"]) if tf_cfg.get("require_vol_confirm", True) else True
    if not (ema_bear and breakdown and rsi_weak and vol_ok):
        return None
    entry = float(last["close"])
    stop = float(e20.iloc[-1])
    return {
        "type": "bear_swing",
        "entry": entry,
        "stop": stop,
...
def trend_bearish_signal(df: pd.DataFrame, cfg: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    breakdown = last["close"] < lowlvl
    rsi_weak = r.iloc[-1] <= tf_cfg.get("rsi_max", 50)
    if not (cross and breakdown and rsi_weak):
        return None
    entry = float(last["close"])
    stop = float(e50.iloc[-1])
    return {
        "type": "bear_trend",
        "entry": entry,
        "stop": stop,
        ...
        "t2": None,
        "level": float(lowlvl),
        "note": "Bearish trend breakdown 1d",
        "event_bar_ts": df.index[-1].isoformat(),
    }
...
def bear_still_valid(df: pd.DataFrame, cfg: Dict[str, Any]) -> bool:
    ...
    if e20.iloc[-1] >= e50.iloc[-1]:
        return False
    if r.iloc[-1] > cfg.get("day", {}).get("bear_invalidate_rsi", 55):
        return False
    return True
...
def silent_dashboard(rds: RedisState):
    ...
    def line(name, k):
        ...
        c = d["closed"]
        w = d["wins"]
        winp = (w / max(1, c)) * 100.0 if c > 0 else 0.0
...
def eth_gate_ok(client: ExClient, tf: str, cfg: Dict[str, Any]) -> Optional[bool]:
    try:
        ...
        if cfg.get("require_ema_stack", True):
            ok &= (e20.iloc[-1] > e50.iloc[-1])
        ok &= (r.iloc[-1] >= cfg.get("min_rsi", 50))
        return bool(ok)
    except Exception:
        return None
...
def movers_detailed_table(rds: RedisState, max_rows: int = 100):
    """
    Print a detailed table of closed Movers trades.

    Shows only the *last max_rows* trades (by opened_at) to avoid flooding logs.
    """
    ...
    if max_rows is not None and max_rows > 0:
        movers = movers[-max_rows:]  # keep only the latest N
...
def run(cfg: Dict[str, Any]):
    ...
    print(f"[universe] Top100 mapped -> {len(top100_pairs)} pairs")
    ...
    for pair in top100_pairs:
        try:
            ...
            if sig and (dayP.day_mtf or {}).get("enabled", True):
                ...
                if ok < need:
                    sig = None
            # ETH gate (Top100 bullish only)
            if sig and eth_cfg.get("enabled", True) and not eth_gate_on:
                sig = None
            if sig:
                sig.update({"symbol": pair, "timeframe": "1h", "exchange": "mexc"})
                if not is_silent_open_top100(rds, pair, "1h", sig["type"]):
                    results_top100.append(sig)
                    record_new_silent(rds, sig, "top100")
        except Exception as e:
            print(f"[scan-top100-day] {pair} err:", e)
    ...
    for pair in top100_pairs:
        try:
            ...
            if sig:
                ...
                if not is_silent_open_top100(rds, pair, tf_swing, sig["type"]):
                    results_top100.append(sig)
                    record_new_silent(rds, sig, "top100")
        except Exception as e:
            print(f"[scan-top100-swing] {pair} err:", e)
    ...
    for pair in top100_pairs:
        try:
            ...
            if sig:
                ...
                if not is_silent_open_top100(rds, pair, "1d", sig["type"]):
                    results_top100.append(sig)
                    record_new_silent(rds, sig, "top100")
        except Exception as e:
            print(f"[scan-top100-trend] {pair} err:", e)
    ...
    if bear_cfg.get("enabled", True) and bear_mode.get("enabled", True):
        ...
        def process_bear_sig(pair: str, timeframe: str, sigb: Dict[str, Any]):
            ...
            display_symbol = inv if inv else underlying
            entry_px = client.last_price(display_symbol) or sigb["entry"]
            sig = {
                "type": "bear",
                "entry": float(entry_px),
                "stop": float(sigb["stop"]) if sigb.get("stop") is not None else None,
                "t1": None,
                "t2": None,
                "level": sigb["level"],
                "note": f"Bearish {underlying} ({timeframe}) — " + (f"inverse: {inv}" if inv else "no inverse token"),
                "event_bar_ts": sigb["event_bar_ts"],
        ...
        for pair in top100_pairs:
            try:
                ...
                process_bear_sig(pair, "1d", trend_bearish_signal(dfd, bear_cfg))
            except Exception as e:
    ...
    for s in all_sigs:
        ...
        if sym in already_open_syms or n_open >= max_pos:
            continue
        if paper_mode:
    ...
    if top100_msg:
        _post_discord(SIG_HOOK, top100_msg)
    if movers_msg:
        _post_discord(SIG_HOOK, movers_msg)
    if bear_msg:
        _post_discord(SIG_HOOK, bear_msg)
    tr_msg = fmt_trade_lines(trade_lines)
    if tr_msg:
        _post_discord(TRD_HOOK, tr_msg)
    ...
    if paper_mode:
        ...
        for sym, pos in sorted(pf_state.get("holdings", {}).items()):
            qty = float(pos["qty"])
            avg = float(pos["avg"])
            last = client.last_price(sym) or avg
            mv_total += qty * last
            pnl_total += (last - avg) * qty
...
# Helpers for silent duplicate checks specific to sources
def is_silent_open_top100(rds: RedisState, symbol: str, tf: str, typ: str) -> bool:
    book = rds.load_json("state", "silent_open", default={})
    return silent_key(symbol, tf, typ, "top100") in book
...
def apply_bear_exits(
    ...
    for k, v in active.items():
        if v.get("symbol") == underlying_pair and v.get("type") in ("day", "swing", "trend"):
            to_close.append(k)
    if not to_close:
        return
