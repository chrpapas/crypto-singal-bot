...
------------------------------------------------
exchanges: ["mexc"]         # will be forced to ["mexc"] by the bot anyway
symbols_watchlist: ["BTC/USDT","ETH/USDT","SOL/USDT"]

bear_mode:
  enabled: true
  inverse_suffixes: ["3S","5S"]   # preference order
...
trading:
  paper: true
  base_balance_usdt: 1000       # initial paper balance (first run only)
  risk_per_trade_pct: 1.0       # risk % per trade (uses stop distance)
  max_concurrent_positions: 10
  min_usdt_order: 10
  live_market_slippage_bps: 10  # live only; 10 bps = 0.10%
...
# (plus your day/swing/trend/bearish/exits/quality/performance sections as before)
------------------------------------------------
"""
...
class ExClient:
    def __init__(self):
        ...
        # Live keys optional (paper by default)
...
# ================== SD zones / helpers (same as yours) ==================
...
# ================== Signals (your same logic, trimmed to essentials) ==================
...
# Bearish detectors (as in your script)
def day_bearish_signal(df: pd.DataFrame, cfg: Dict[str,Any], sd_cfg: Dict[str,Any], zones=None):
    ...
    # make it look like a long entry template (so we can size/track identically)
    entry = float(last["close"]); stop = float(last["high"])  # conservative (can also use invalidation)
...
def compute_qty_for_risk(entry: float, stop: float, equity_usdt: float, risk_pct: float) -> float:
    # qty so that (entry - stop) * qty = equity * risk_pct%
...
def run(cfg: Dict[str,Any]):
    ...
    for pair in scan_pairs_day:
        try:
            ...
            if not sig:
                er = dayP.early_reversal or {}
                if er.get("enabled", False):
                    # reuse same ER function body inline (slim): skip here to keep short
                    pass
    ...
    # Load active holdings count
    open_syms = set(positions.get("active_positions", {}).keys())
    ...
    for sig in results["signals"]:
        ...
        already = any((sym in k) for k in positions.get("active_positions",{}).keys())
        if already:
            continue
        if n_open >= max_pos:
        ...
        # Sizing (paper uses equity = cash + holdings MTM; but for simplicity start with cash)
        equity = pf["cash_usdt"] if paper_mode else 0.0
        qty = compute_qty_for_risk(entry, stop, equity, risk_pct) if paper_mode else 0.0
        if paper_mode:
            ...
            # live: get min amount via markets if needed; apply slippage up to slip_bps
            try:
    ...
    if results["signals"]:
        for s in results["signals"]:
            print(f"[ENTRY] {s['symbol']} {s['timeframe']} — {s['type']} — {s['note']} | entry {s['entry']:.6f} stop {s['stop']:.6f} t1 {s.get('t1')} t2 {s.get('t2')}")
    else:
